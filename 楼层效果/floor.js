		...
		for(let i=0;i<this.lis.length;i++){
			...
			this.lis[i].onclick=function(){
				...
				/*if(document.documentElement.scrollTop){
					move(document,{document.documentElement.scrollTop:that.floor[i].offsetTop});
				}else{//scrollTop没有单位，用不了move函数
					move(document,{document.body.scrollTop:that.floor[i].offsetTop});
				}*/
			}
		}
